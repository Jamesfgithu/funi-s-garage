...
export async function getUserSafelistLinks(userId: string): Promise<SafelistLink[]> {
  try {
    const q = query(collection(db, 'safelistLinks'), where('userId', '==', userId));
...
export function calculateReadiness(lastSubmitted: Date | null, frequency: number) {
...
export async function getDashboardStats(userId: string) {
  try {
    ...
    const readyLinks = links.filter(link => {
      const readiness = calculateReadiness(link.lastSubmitted ?? null, link.frequency);
    ...
    return {
      ...
      activeLinks,
      readyLinks,
      submissionRate: totalLinks > 0 ? Math.round((readyLinks / totalLinks) * 100) : 0,
      readyNow: readyLinks,
      readySoon: soonLinks,
...
export async function addSafelistLink(userId: string, linkData: Omit<SafelistLink, 'id' | 'userId' | 'createdAt'>) {
  try {
    ...
    console.error('Error adding safelist link:', error);    return { success: false, error: (error as Error).message };
  }
}

export async function updateLastSubmitted(linkId: string, timestamp: Date = new Date()) {
  try {

...
export function LoginForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!auth || !auth.app) {
        console.error(
          'LoginForm: Firebase auth or auth.app is not initialized!'
        );
        userFriendlyMessage =
      ...
      console.log(
        'LoginForm: Firebase auth instance config being used:',
        JSON.stringify(auth.app.options, null, 2)
      );
      console.log(
        `LoginForm: Attempting Firebase sign-in with email: ${email}`
      );
      ...
      if (!user) {
        userFriendlyMessage =
          'Login failed: No user object returned from Firebase.';
        console.error(
          'LoginForm: No user object returned from signInWithEmailAndPassword despite no error throw. This should not happen.'
        );
        throw new Error(userFriendlyMessage);
      }
      console.log(
        `LoginForm: Firebase sign-in successful for user: ${user.uid}, email: ${user.email}`
      );

      if (!user.emailVerified) {
        userFriendlyMessage =
          'Please verify your email before logging in. Check your inbox or resend verification from the signup/verify page.';
        ...
        console.log(
          `LoginForm: User email ${user.email} is verified. Proceeding to dashboard.`
        );
        router.push('/dashboard');
      }
      console.log(
        'LoginForm: Exiting main try block (after success/email verification check).'
      );
    } catch (e: any) {
      ...
      if (e && typeof e === 'object' && 'code' in e && 'message' in e) {
        ...
        let specificErrorMessage = firebaseError.message
          .replace('Firebase: ', '')
          .replace(/\(auth\/[^)]+\)\.?/, '')
          .trim();
        ...
        switch (firebaseError.code) {
          ...
          case 'auth/too-many-requests':
            userFriendlyMessage =
              'Access to this account has been temporarily disabled due to many failed login attempts. You can try again later or reset your password.';
            break;
          case 'auth/network-request-failed':
            userFriendlyMessage =
              'A network connection error occurred. Please check your internet connection and try again. Unable to reach Firebase servers.';
            break;
          case 'auth/operation-not-allowed':
            userFriendlyMessage =
              'Email/password sign-in is not enabled for this project. Please contact support.';
            break;
          default:
      ...
      setIsLoading(false);
      console.log(
        'LoginForm: setIsLoading(false) called in finally - spinner should be hidden.'
      );
    }
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              disabled={isLoading}
              autoComplete="email"
              className="border-gray-300 bg-white text-black dark:border-gray-600 dark:bg-gray-800 dark:text-white"
            />
          </div>
          <div className="space-y-2">

...
export default function SafelistManagementPage() {
  ...
  function handleCustomFrequencyChange(value: string) {
    ...
    if (value.trim()) {
      ...
      if (parsed !== null) {
        ...
        setCustomError(
          'Format: "X hours" or "X days" (e.g., "6 hours", "3 days")'
        );
      }
    }
  }
  ...
  async function handleAddLink() {
    ...
    if (!user) return;
    const response = await addSafelistLink(
      { ...newLink, createdAt: new Date(), userId: user.uid },
      user.uid
    );
  ...
  const selectedFrequency = frequencyOptions.find(
    (opt) => opt.value === newLinkFrequency
  );
  const displayLabel = isCustomSelected
    ? `Custom: ${customFrequency || 'Enter frequency'}`
    : selectedFrequency?.label || 'Every 7 days';

  // For edit form
  const editSelectedFrequency = frequencyOptions.find(
    (opt) => opt.value === editForm.frequency
  );
  const editDisplayLabel = editIsCustomSelected
    ? `Custom: ${editCustomFrequency || 'Enter frequency'}`
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-col gap-4 sm:flex-row">
              <input
                ...
                className="bg-background text-foreground flex-1 rounded border p-2"
              />
              <input
                type="url"
            ...
            <div className="flex flex-col gap-4 sm:flex-row">
              ...
              <div className="relative flex-1">
                <button
                  ...
                  className="bg-background text-foreground hover:bg-muted flex w-full items-center justify-between rounded border p-2"
                >
                ...
                {dropdownOpen && (
                  <div className="bg-card absolute top-full right-0 left-0 z-10 mt-1 max-h-60 overflow-y-auto rounded border shadow-lg">
                    {frequencyOptions.map((option) => (
                      <button
                        key={option.value}
                        type="button"
                        onClick={() =>
                          handleFrequencySelect(option.value.toString())
                        }
                        className="hover:bg-muted text-foreground border-border w-full border-b p-2 text-left last:border-b-0"
                      >
                        {option.label}
                      </button>
                    ))}
            ...
            <button
              ...
              disabled={
                isCustomSelected && (!!customError || !customFrequency.trim())
              }
            >
      ...
      {loadingLinks ? (
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Add your first link above to get started with safelist management.
            </CardDescription>
          </CardHeader>
        </Card>
      ) : (
        <div className="space-y-4">
          {links.map((link) => {
            ...
            const { statusColor, statusIcon, statusText, pulseClass } =
              getStatusInfo(link);
            ...
            return (
              ...
              >
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        ...
                        <div
                          className={`flex items-center gap-1 ${statusColor} ${pulseClass}`}
                        >
                          {statusIcon}
                  ...
                  {isEditing && (
                    <div
                      className="bg-muted mt-4 space-y-4 rounded-lg p-4"
                      ...
                      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                        <div>
                          ...
                          <input
                            ...
                            className="bg-background text-foreground w-full rounded border p-2 text-sm"
                          />
                        </div>
                        ...
                        <div>
                          ...
                          <div className="relative">
                            <button
                              ...
                              className="bg-background text-foreground hover:bg-muted flex w-full items-center justify-between rounded border p-2 text-sm"
                            >
                            ...
                            {editDropdownOpen && (
                              <div className="bg-card absolute top-full right-0 left-0 z-20 mt-1 max-h-48 overflow-y-auto rounded border shadow-lg">
                                {frequencyOptions.map((option) => (
                                  <button
                                    key={option.value}
                                    type="button"
                                    onClick={() =>
                                      handleEditFrequencySelect(
                                        option.value.toString()
                                      )
                                    }
                                    className="hover:bg-muted text-foreground border-border w-full border-b p-2 text-left text-xs last:border-b-0"
                                  >
                                    {option.label}
                                  </button>
                                ))}
                          ...
                          {editIsCustomSelected && (
                            <div className="mt-2">
                              <input
                                ...
                                onChange={(e) =>
                                  handleEditCustomFrequencyChange(
                                    e.target.value
                                  )
                                }
                                className={`bg-background text-foreground w-full rounded border p-2 text-sm ${

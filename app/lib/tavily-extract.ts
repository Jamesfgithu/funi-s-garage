...
export async function analyzeWebContent(
  extractedContent: TavilyExtractResponse
): Promise<AnalyzedWebContent> {
  ...
  try {
    ...
    throw new Error(
      `Failed to analyze webpage content: ${error instanceof Error ? error.message : 'Unknown error'}`
    );
  }
}

// Enhanced intelligent content analysis with better text cleaning
async function intelligentContentAnalysis(
  content: TavilyExtractResponse
): Promise<AnalyzedWebContent> {
...
function extractProductName(title: string, content: string): string {
  ...
  const productMatches = content.match(
    /(?:introducing|presenting|get|buy|order)\s+([A-Z][^.!?]*?)(?:\s+(?:system|course|training|program|guide|method))?/i
  );
  ...
  benefitPatterns.forEach((pattern) => {
    ...
    if (matches) {
      matches.slice(0, 3).forEach((match) => {
        const benefit = match
          .replace(
            /^(?:you'll|you will|you can|you get|get|achieve|increase|improve|boost|generate|discover|learn|helps? you|allows you to|enables you to)\s+/i,
            ''
          )
          .replace(/[.!?]$/, '')
          .trim();
